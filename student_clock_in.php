...
if(isset($_GET['username'])){
        ...
        //query user's most recent attendance details 
        // $recent_record_id = "(SELECT MAX(id) FROM attendance WHERE anumber = '$anumber' AND date = CURDATE() AND time_in = time_out)";


        //  //query for user's most recent attendance details 
        // $sql = "SELECT  id, time_in, time_out FROM attendance WHERE id = (SELECT MAX(id) FROM attendance WHERE anumber = '$anumber' AND date = CURDATE() ";

        // $result2 = mysqli_query($link, $sql);

        // //if there's any record for recent attendance for current date where user is still logged in, fetch the id and memo
        //  if(empty($result2)){
        ...
        //         $id = $row2['id']; //id for recent record
        //         $memo = "YES"; //memo in order to update it (add clocked out under which admin and total time spent)
                

        //         //then clock them out
        //         if($row2['time_in']==$row2['time_out']){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                   ...
                   if($type == "instructor"){
                     header("location: instructor_dashboard.php?anumber=$anumber");
                   }
